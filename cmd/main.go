...
func main() {
	...
	if err := (&controller.ConfigMapBuilderReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorder("configmapbuilder"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "configmapbuilder")
		os.Exit(1)
	}
