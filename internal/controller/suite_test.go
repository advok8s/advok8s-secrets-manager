...
var _ = BeforeSuite(func() {
	...
	err = configmapsv1beta1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	err = (&ConfigMapCopierReconciler{
		Client:   k8sManager.GetClient(),
		Scheme:   k8sManager.GetScheme(),
		Recorder: k8sManager.GetEventRecorder("configmapcopier"),
	}).SetupWithManager(k8sManager)
	Expect(err).ToNot(HaveOccurred())
	...
	err = (&ConfigMapBuilderReconciler{
		Client:   k8sManager.GetClient(),
		Scheme:   k8sManager.GetScheme(),
		Recorder: k8sManager.GetEventRecorder("configmapbuilder"),
	}).SetupWithManager(k8sManager)
	Expect(err).ToNot(HaveOccurred())
